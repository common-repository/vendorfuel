...
function controller(
			...
			Utils.getHttpPromise(req)
				.then(
					(resp: any) => {
						...
						$scope.newGroup.default_price_sheet =
	...
	$scope.CustomerSearchResults = (resp: any) => {
		if (
			$scope.newGroup.customers.data &&
			$scope.newGroup.customers.data.length
		) {
			const customerIds = $scope.newGroup.customers.data.map(
				(c: any) => c.customer_id
			);
