...
export const CustomerCopyButtonComponent: ng.IComponentOptions = {
	...
	controller: class CustomerCopyButtonController {
		...
		public passwordHas = {
			all: ( password: string ) => {
				return this.passwordHas.length( password ) &&
		...
		constructor( private CustomersService: any ) {
			this.modal = new bootstrap.Modal( document.getElementById( 'copyCustomerModal' ) );
		}
		...
		copyCustomer() {
			...
			this.CustomersService.copy( this.customerId, data )
				.then( ( response:any ) => {
