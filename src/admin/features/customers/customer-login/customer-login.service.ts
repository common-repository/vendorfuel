export class CustomerLoginService {
  static $inject: string[] = ["$cookies", "$http", "$window", "Localized"];
  ...
  private updateCookies(data: {
    ...
    if (data.group) {
      this.$cookies.put("vf.user.group", JSON.stringify(data.group), options);
    }
    ...
    if (typeof data.approver !== "undefined") {
      this.$cookies.put("vf.user.approver", data.approver.toString(), options);
    }
    if (data.cart_count) {
      this.$cookies.put(
        "vf.user.cartCount",
    ...
    if (data.group_id) {
      this.$cookies.put("vf.user.group_id", data.group_id.toString(), options);
      this.$cookies.put(
        "vf.user.currentGroup_id",
