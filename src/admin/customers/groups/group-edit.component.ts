...
function controller(
	...
	this.$onInit = () => {
		this.breadcrumbs = [
			...
			{ label: 'Groups', href: '?page=vf-customers#/groups' },
		];
	...
	this.showGroup = () => {
		const url = `${nextApiURL}/admin/customers/groups/${$stateParams.id}`;
		const config = {
			params: {
				page: this.page,
	...
	$scope.CustomerSearchResults = (resp: any) => {
		const currentIds = this.group.customers.data.map(
			(c: any) => c.customer_id
		);
		for (let i = 0; i < resp.length; i++) {
			if (
				...
				for (let j = 0; j < this.group.customers.data.length; j++) {
					if (
						this.group.customers.data[j].customer_id ===
						resp[i].value.customer_id
					) {
						this.group.customers.data.splice(j, 1);
						break;
					}
				}
			}
			if (!currentIds.includes(resp[i].value.customer_id)) {
				this.group.customers.data.push(resp[i].value);
			}
		}
	};
		...
		Utils.getHttpPromise(req)
			.then(
				(response: any) => {
					...
					this.group.shipping_flat_rates = [];
		...
		const data = {
			tabs: [
				...
				},
				{
					http: {
						method: 'GET',
						url: this.groupEndpoint + this.group.group_id,
						params: {
							q: '',
						},
					},
	...
	$scope.OpenRatesSearchModal = () => {
			...
			tabs: [
				{
					http: {
						...
						params: {
							...
							excludedId: this.group.group_id,
		...
		//callback object (confirm, cancel), title, message, optionCancel, optionConfirm, httpConfig
		SearchModal.Show(callback, 'Rates', data, 'Add Shipping Rates');
	};
	$scope.rateResults = (resp: any) => {
		...
		resp.forEach((rate: any) => {
			if (rate.action === 'add') {
				...
				this.group.shipping_flat_rates.push({
					id: rate.value.id,
				});
			}
			if (rate.action === 'unselect') {
				angular.forEach(this.group.shipping_flat_rates, (val, key) => {
					if (val.id === rate.value.id) {
						this.group.shipping_flat_rates.splice(key, 1);
					}
				});
			}
				...
				(resp: any) => {
					if (resp.errors.length <= 0) {
						angular.forEach($scope.updatedRates, (rate) => {
							if (rate.action === 'add') {
								this.group.flatrates.push(rate.value);
							} else {
								angular.forEach(
									this.group.flatrates,
									(val, key) => {
										if (val.id === rate.value.id) {
											this.group.flatrates.splice(key, 1);
										}
									}
								);
							}
	...
	this.deleteGroup = () => {
		...
		const endpoint = this.groupEndpoint + this.group.group_id;
	...
	$scope.OpenSupplierSearch = () => {
		...
		callback = {
			confirm(resp: any) {
				this.group.punchout_profiles.push({
					supplier: {
	...
	$scope.RemoveProfile = (index: number) => {
		this.group.punchout_profiles[index].deleted = true;
	};
