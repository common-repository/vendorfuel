import type { Localized } from '../../../admin/types';
...
(function () {
	...
	angular.module('vfApp').factory('catalogService', catalogService);
	 ...
	 * @return {Object} Factory service
	 */
	function catalogService($location, $rootScope, Products) {
		const sortByOptions = [
			{
				name: 'Most Relevant',
				value: 'relevance',
		...
		function getCatalog() {
			...
			return Products.list(params)
				...
				.then((data) => {
					...
					catalog.collection = data.collection
						? data.collection
						: null;
					catalog.currentPage = data.currentPage
						? data.currentPage
						: 1;
					...
					catalog.hits = data.product_briefs
						? data.product_briefs
						: null;
					catalog.numResults = data.num_results
						? data.num_results
						: 0;
		...
		function getParams() {
			...
			const params = {
				...
				rpp: 32,
				pcrc:
					search.recycled || search.recycled === true ? true : false,
				q: search.q,
		...
		function changeRefinement(refinement) {
			if (refinement.value === true) {
				...
				const refinementsIdx =
					// eslint-disable-next-line max-len
					catalog.refinements.findIndex(
						(item) =>
							item.facet === refinement.facet &&
							item.key === refinement.key
					);
				...
				if (
					...
					if (facetIdx >= 0) {
						...
						catalog.attributes[facetIdx].values[
							attrIdx
						].value.checked = false;
					}
				}
		...
		function setCategories(data) {
			...
			if (data.category) {
				...
				if (activeCatIndex > -1) {
					...
					categories[activeCatIndex].subcategories =
						data.sub_categories;
				}
			}
			return categories;
		...
		function setFilters(filters) {
			return {
				...
				isShowingAbilityOne:
					filters && filters.ability_one ? true : false,
			};
		}
