...
angular
	.module( 'vfApp' )
	.component( 'checkoutFinalizeOrder', {
		bindings: {
...
FinalizeOrderController.$inject = [
	'Alerts',
	'Checkout',
	'Utils',
];

function FinalizeOrderController(
	Alerts,
	...
	function confirmOrder() {
		...
		const params = {
			...
			billing_first_name:
					vm.isShippingSameAsBilling ? vm.shippingAddress.first_name : vm.billingAddress.first_name,
			billing_last_name:
					vm.isShippingSameAsBilling ? vm.shippingAddress.last_name : vm.billingAddress.last_name,
			billing_email: vm.isShippingSameAsBilling ? vm.shippingAddress.email : vm.billingAddress.email,
			billing_address1:
					vm.isShippingSameAsBilling ? vm.shippingAddress.address1 : vm.billingAddress.address1,
			billing_address2:
					vm.isShippingSameAsBilling ? vm.shippingAddress.address2 : vm.billingAddress.address2,
			billing_city: vm.isShippingSameAsBilling ? vm.shippingAddress.city : vm.billingAddress.city,
			billing_state: vm.isShippingSameAsBilling ? vm.shippingAddress.state.id : vm.billingAddress.state.id,
			billing_zip: vm.isShippingSameAsBilling ? vm.shippingAddress.zip : vm.billingAddress.zip,
			billing_phone: vm.isShippingSameAsBilling ? vm.shippingAddress.phone : vm.billingAddress.phone,
	...
	this.completePriceAvailability = ( data: any ) => {
		...
		Checkout.priceAvailabilityResponse( order, priceAvailabilityResponse, reconfirm )
			.then( ( response: any ) => {
	...
	function getExtraFields( extraFields ) {
		return Object.fromEntries( Object.entries( extraFields ).map( ( element: any ) => {
	...
	function getProp65Warning( warning ) {
		...
		warning = warning.replace( /(www.p65warnings.ca.gov)/gi, '<a href="https://$&" target="_blank">$&</a>' );
		return warning;
	}

	this.setHasAdditionShipping = ( items: {additional_shipping: boolean}[] ) => {
		this.hasAdditionalShipping = Object.values( items ).some( ( item ) => item.additional_shipping );
	};
}
