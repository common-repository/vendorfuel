...
export const CartItems: ng.IComponentOptions = {
	template,
	bindings: {
		isLoading: '<',
		cart: '<',
...
controller.$inject = ['Cart', 'User', 'Utils'];

function controller(Cart, User, Utils) {
	const vm = this;
	...
	vm.$onChanges = (changes) => {
		if (changes.cart && changes.cart.currentValue) {
			...
			for (const item in vm.cart.items) {
				if (item) {
					vm.cart.items[item].prop65Warning = getProp65Warning(
						vm.cart.items[item]
					);
				}
			}
	...
	function remove(productId) {
		Cart.remove(productId).then(function () {
