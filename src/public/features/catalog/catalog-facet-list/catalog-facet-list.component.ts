...
export const CatalogFacetList: ng.IComponentOptions = {
	bindings: {
		facetName: '@',
		facetTitle: '@',
	},
...
controller.$inject = ['$location', '$rootScope', 'Products', 'catalogService'];
...
function controller(
	 ...
	 */
	function onClickApply(key: string, form) {
		const checkedValues = vm.facets.filter((element) => element.isChecked);
		const uncheckedValues = vm.facets.filter(
	...
	const hasInStockEnabled = () => {
		const search = $location.search();
		return search.available_stock === 'false' ||
			search.available_stock === false
			? false
			: true;
	};
	...
	this.searchFacets = (event: Event, query: string) => {
		if (event.type === 'click' || event.keyCode === 13) {
			...
			this.hasStockOrRecycledApplied =
				hasInStockEnabled() || hasRecycledEnabled();
			...
			Products.list(params)
				...
				.then((data) => {
					this.facets = getFacetSearchResults(
						data.facet_search_results,
						this.facetName
					);
