...
export function cartService(
	...
	// TODO: Good integration with api so cart and service have the same data.
	// TODO: Setup watching sessionStorage variable for changes, and updating in other tabs...
	const self = this;
	self.details = null;
	...
	self.selectedShipping = getIntFromCookie( $cookies.get( 'vf.cart.selectedShipping' ) );
	...
	const setSelectedShippingToDefault = function setSelectedShippingToDefault() {
		for ( const id in self.shipping_methods ) {
			if ( Object.prototype.hasOwnProperty.call( self.shipping_methods, id ) ) {
				Debug.log( id );
	...
	self.addItems = function( products ) {
		...
		return loginCheck.then( function( resp ) {
	...
	self.remove = function( productId ) {
		return $http.post( localized.apiURL + '/cart/remove', {
	...
	self.update = function( qty, product_id ) {
		...
		return $http.post( localized.apiURL + '/cart/update', {
	...
	self.clear = function() {
		return $http.post( localized.apiURL + '/cart/clear' ).then( function( resp ) {
			self.updateFromApi( resp.data );
			return resp;
		} );
	};
	...
	self.fillDetails = function() {
		...
		return $http.get( formatURLParams( localized.apiURL + '/cart/view', params ) ).then( function( resp ) {
			self.updateFromApi( resp.data );

			if ( ! self.selectedShipping ) {
				setSelectedShippingToDefault();
			} // TODO: if multiple tax_counties in resp, show pick county dialog...

			return resp;
		} );
	}; // TODO: Move saved cart functionality to a seperate 'SavedCart' service.?
	...
	self.getSaved = function( savedCartId ) {
		return $http.get( formatURLParams( localized.apiURL + '/cart/saved/view', {
	...
	self.getFavorites = function() {
		return $http.post( localized.apiURL + '/cart/favorites/view', {} ).then( function( resp ) {
			return resp;
		} );
	};

	self.addFavorite = function( params ) {
		return $http.post( localized.apiURL + '/cart/favorites/add', params ).then( function( resp ) {
			return resp;
		} );
	};

	self.removeFavorite = function( params ) {
		return $http.post( localized.apiURL + '/cart/favorites/remove', params ).then( function( resp ) {
			return resp;
		} );
	};
	/**
	...
	self.addPromoCode = function( promoCode ) {
		...
		return $http.post( localized.apiURL + '/cart/promo-code/add', {
	...
	self.removePromoCode = function( promoCodeId ) {
		return $http.post( localized.apiURL + '/cart/promo-code/remove', {
	...
	self.addBySku = function( sku, qty ) {
		qty = parseInt( qty ) || 1;
		return $http.post( localized.apiURL + '/cart/add-sku', {
			sku,
			qty,
		} ).then( function( resp ) {
			Debug.log( resp );
	...
	self.getShippingMethods = function( order_id, profile_id, zipcode ) {
		...
		return $http.post( localized.apiURL + '/cart/order/shipping/methods', params ).then( function( resp ) {
			return resp;
		} );
	};
	...
	self.checkout = function( paypal, return_url, cancel_url, shipping_id ) {
		...
		return $http.post( localized.apiURL + '/cart/checkout', params ).then( function( resp ) {
	...
	self.priceAvailabilityResponse = function( order, price_availability_response, reconfirm ) {
		const params = {};
		...
		return $http.post( localized.apiURL + '/cart/order/price-availability', params ).then( function( resp ) {
			return resp;
		} );
	};
	...
	self.confirmOrder = function( checkout_info ) {
		const params = {};
		params.shipping_id = checkout_info.selected_shipping_profile.shipping_id;
		...
		if ( localized.settings.general.checkout.cost_center_option ) {
			params.cost_center_code = checkout_info.extraFields.cost_center_code;
		}
		...
		return $http.post( localized.apiURL + '/cart/order/confirm', params ).then( function( resp ) {
			return resp;
		} );
	};
	...
	self.completeOrder = function( params ) {
		return $http.post( localized.apiURL + '/cart/order/complete', params ).then( function( resp ) {
	...
	self.payfabricUrl = function( return_url, order_id ) {
		...
		return $http.post( localized.apiURL + '/cart/order/payfabricUrl', params ).then( function( resp ) {
